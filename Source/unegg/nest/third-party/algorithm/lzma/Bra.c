...
SizeT ARMT_Convert(Byte *data, SizeT size, UInt32 ip, int encoding)
{
    ...
    for (i = 0; i <= size; i += 2)
    {
        ...
        {
            UInt32 dest;
            UInt32 src = (((UInt32) data[i + 1] & 0x7) << 19) | ((UInt32) data[i + 0] << 11) | (((UInt32) data[i + 3] & 0x7) << 8)
                | (data[i + 2]);
...
SizeT PPC_Convert(Byte *data, SizeT size, UInt32 ip, int encoding)
{
    ...
    for (i = 0; i <= size; i += 4)
    {
        ...
        {
            UInt32 src = ((UInt32) (data[i + 0] & 3) << 24) | ((UInt32) data[i + 1] << 16) | ((UInt32) data[i + 2] << 8) | ((UInt32) data[i
                + 3] & (~3));
